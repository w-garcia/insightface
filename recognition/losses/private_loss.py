...
def p_softmax_loss(dense, labels):
    # cce = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)  # do softmax
    # loss = cce(labels, dense)
    vector_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=dense)

    return vector_loss


def p_center_loss(features, label, centers, alpha):
    ...
    vector_loss = tf.reduce_mean(tf.square(features - centers_batch), axis=1)
    return vector_loss, centers


def p_arcface_loss(x, normx_cos, labels, m1, m2, m3, s):
    ...
    vector_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=prelogits)

    return vector_loss


def p_triplet_loss(anchor_emb, pos_emb, neg_emb, alpha):
    ...
    vector_loss = tf.maximum(basic_loss, 0.0)
